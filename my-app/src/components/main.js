...
import './main.css'; // Import CSS file with background image styling
import head from '../screens/header-img.svg';

const MainPage = () => {
  const [textIndex, setTextIndex] = useState(0);
  const [currentText, setCurrentText] = useState('');
  const [isDeleting, setIsDeleting] = useState(false);
  const texts = ["machine learning enthusiast", "web developer"];
